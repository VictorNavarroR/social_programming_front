...
export default function BlogList({post}) {
  ...
  return (
    <div className="post">
        ...
        <div className="post__meta">Categorías:{post.categories.map(cat => cat).join('-')} <i className="fa-solid fa-user"></i> <strong>Creado por:</strong> {userName} | <i className="fa-solid fa-calendar-days"></i> <strong>En fecha</strong>: {convertDate(post.createdAt)}</div>
        <h2>{ post.title }</h2>
